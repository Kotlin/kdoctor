...
class GradleProjectDiagnostic(
    ...
    override suspend fun diagnose(): Diagnosis {
        ...
        val projectPathExists = projectPathFiles != null
        if (!projectPathExists) {
            result.addFailure("Path is not found")
            return result.build()
        }
        if (!projectPathFiles.orEmpty().contains("settings.gradle")) {
            result.addFailure(
        ...
        system.rm(initScript) //result code doesn't matter

        val initScriptRenamed = system.mv(tempFile, initScript)
        if (!initScriptRenamed) {
            result.addFailure(
                "Error: impossible to prepare $initScript file",
                "Check your file system write permissions"
            )
            return result.build()
        }

        system.logger.d("Created $initScript")
            ...
            if (version == "null") {
                val pluginArtifact = artifacts.firstOrNull { it.name == "$id.gradle.plugin" }
                    ?: if (id.startsWith("org.jetbrains.kotlin.")) {
                        ...
                        system.logger.d("Unknown plugin $id")
                        null
                    }
